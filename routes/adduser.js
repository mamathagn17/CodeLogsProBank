...
router.post("/adduser", async (req, res) => {
  ...
  const fs = require('fs');
  router.get('/downloadUserLogs', async (req, res) => {
    try {
      const pool = await sql.connect(config);
      const path = require('path');
  
      const result = await pool.request().query('SELECT * FROM tb_useraddlogs');
      const UserLogs = result.recordset;
      const csvData = UserLogs.map(log => Object.values(log).join(','));
      const tempDir = path.join(__dirname, '..', 'temp');
      if (!fs.existsSync(tempDir)) {
        fs.mkdirSync(tempDir);
      }
      const tempFilePath = path.join(tempDir, 'User_logs.csv');
      fs.writeFileSync(tempFilePath, csvData.join('\n'));
  
      res.download(tempFilePath, 'User.csv', () => {
